...
import { generateSlug } from './story.element.utils';
...
export abstract class StoryElementCrudService<T extends StoryElement> {
  ...
  async findById(id: string, include?: string[]): Promise<T | undefined> {
  ...
  async update(dto: Partial<T>): Promise<T> {
    const { _id } = dto;
  ...
  protected populateWithIncludes(query, include?: string[]) {
    include = include ?? [];
  ...
  protected createSortQuery(sort: string[] | undefined): {
